...
%     figure(figureIndex),clf,set(gcf,'Name','Putative Figure'); figureIndex = figureIndex + 1;
   ...
   for RANSACIteration = 1:100
   % Select 2 feature points
   p1Index = randi(length(p1));
   p2Index = randi(length(p2));
   ...
   noOfInliers = length(inliersInThisH_k);
   if (noOfInliers > maxNoOfInliers)
       H_k_best = H_k
       noOfInliers
       maxNoOfInliers = noOfInliers;
   end
